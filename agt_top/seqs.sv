...
task fd_seq_1::body();
  ...
  start_item(req);
  assert(req.randomize() with {PADDR == 3; PWRITE == 1; PWDATA == 8'b1000_0000;});
  ...
  start_item(req);
  assert(req.randomize() with {PADDR == 1; PWRITE == 1; PWDATA == 8'b0000_0000;});
    //`uvm_info("full_duplex_seqs_1",$sformatf("Printing from uart_sequence \n %s",req.sprint()),UVM_LOW)
  finish_item(req);

  // 3 DL LSB = 54[0011_0110] so, the selecting of special register DL[MSB]
  start_item(req);
  assert(req.randomize() with {PADDR == 0; PWRITE == 1; PWDATA == 8'b0011_0110;});
    //`uvm_info("full_duplex_seqs_1",$sformatf("Printing from uart_sequence \n %s",req.sprint()),UVM_LOW)
  finish_item(req);

  // 4 LCR[7] = 0 so, the selecting of general register and [1,0] = 11 --> 8 data bits
  start_item(req);
  assert(req.randomize() with {PADDR == 3; PWRITE == 1; PWDATA == 8'b0000_0011;});
    //`uvm_info("full_duplex_seqs_1",$sformatf("Printing from uart_sequence \n %s",req.sprint()),UVM_LOW)
  finish_item(req);

  // 5 IER [Received Data available interrupt] 0 bit = 1 enabled
  start_item(req);
  assert(req.randomize() with {PADDR == 1; PWRITE == 1; PWDATA == 8'b0000_0001;});
    //`uvm_info("full_duplex_seqs_1",$sformatf("Printing from uart_sequence \n %s",req.sprint()),UVM_LOW)
  finish_item(req);

  // 6 FCR [Received Data available interrupt] [10] bit = 11 [Tx,Rx]
  start_item(req);
  assert(req.randomize() with {PADDR == 2; PWRITE == 1; PWDATA == 8'b0000_0110;});
    //`uvm_info("full_duplex_seqs_1",$sformatf("Printing from uart_sequence \n %s",req.sprint()),UVM_LOW)
  finish_item(req);

  // 7 THR 
  start_item(req);
  assert(req.randomize() with {PADDR == 0; PWRITE == 1; PWDATA == 8'b0000_0111;});
    //`uvm_info("full_duplex_seqs_1",$sformatf("Printing from uart_sequence \n %s",req.sprint()),UVM_LOW)
  finish_item(req);

  // 8 IIR 
  start_item(req);
  assert(req.randomize() with {PADDR == 2; PWRITE == 0; });
    //`uvm_info("full_duplex_seqs_1",$sformatf("Printing from uart_sequence \n %s",req.sprint()),UVM_LOW)
  finish_item(req);
  ...
  begin
   // $display("The Value Stored In IIR is %b", req.IIR);
    start_item(req);
    assert(req.randomize() with {PADDR == 0; PWRITE == 0; });
      //`uvm_info("full_duplex_seqs_1",$sformatf("Printing from uart_sequence \n %s",req.sprint()),UVM_LOW)
    finish_item(req);
  end
  ...
  begin
   // $display("The Value Stored In IIR is %b", req.IIR);
    start_item(req);
    assert(req.randomize() with {PADDR == 5; PWRITE == 0; });
      //`uvm_info("full_duplex_seqs_1",$sformatf("Printing from uart_sequence \n %s",req.sprint()),UVM_LOW)
    finish_item(req);
...
task fd_seq_2::body();
  ...
  start_item(req);
  assert(req.randomize() with {PADDR == 1; PWRITE == 1; PWDATA == 8'b0000_0000;});
  //`uvm_info("full_duplex_seqs_2",$sformatf("Printing from uart_sequence 0n%s",req.sprint()),UVM_LOW)
  finish_item(req);

  // 3 DL LSB = 54[0011_0110] so, the selecting of special register DL[MSB]
  start_item(req);
  assert(req.randomize() with {PADDR == 0; PWRITE == 1; PWDATA == 8'b0001_1010;});
    //`uvm_info("full_duplex_seqs_2",$sformatf("Printing from uart_sequence \n %s",req.sprint()),UVM_LOW)
  finish_item(req);
  ...
  // 7 THR 
  start_item(req);
  assert(req.randomize() with {PADDR == 0; PWRITE == 1; PWDATA == 8'b0000_1010;});
    //`uvm_info("full_duplex_seqs_2",$sformatf("Printing from uart_sequence \n %s",req.sprint()),UVM_LOW)
  finish_item(req);
...
task lb_seq_1::body();
  ...
  // 7 MSR for loop back
  start_item(req);
  assert(req.randomize() with {PADDR == 4; PWRITE == 1; PWDATA == 8'b0001_0000;});
  finish_item(req);
...
task parity_1::body();
  ...
  // 4 LCR[7] = 0 so, the selecting of general register and [1,0] = 11 --> 8 data bits
  start_item(req);
  assert(req.randomize() with {PADDR == 3; PWRITE == 1; PWDATA == 8'b0001_1011;});
  finish_item(req);

  // 5 IER [Received Data available interrupt] 0 bit = 1 enabled
  start_item(req);
  assert(req.randomize() with {PADDR == 1; PWRITE == 1; PWDATA == 8'b0000_0100;});
  finish_item(req);
...
task parity_2::body();
  ...
  // 4 LCR[7] = 0 so, the selecting of general register and [1,0] = 11 --> 8 data bits
  start_item(req);
  assert(req.randomize() with {PADDR == 3; PWRITE == 1; PWDATA == 8'b0000_1011;});
  finish_item(req);
  ...
  // 7 THR 
  start_item(req);
  assert(req.randomize() with {PADDR == 0; PWRITE == 1; PWDATA == 8'b0000_1011;});
  finish_item(req);
...
task fr_seq_1::body();
  ...
  // 4 LCR[7] = 0 so, the selecting of general register and [1,0] = 11 --> 8 data bits
  start_item(req);
  assert(req.randomize() with {PADDR == 3; PWRITE == 1; PWDATA == 8'b0000_0100;});
    //`uvm_info("full_duplex_seqs_1",$sformatf("Printing from uart_sequence \n %s",req.sprint()),UVM_LOW)
  finish_item(req);
...
task fr_seq_2::body();
  ...
  // 4 LCR[7] = 0 so, the selecting of general register and [1,0] = 11 --> 8 data bits
  start_item(req);
  assert(req.randomize() with {PADDR == 3; PWRITE == 1; PWDATA == 8'b0000_0111;});
    //`uvm_info("full_duplex_seqs_2",$sformatf("Printing from uart_sequence \n %s",req.sprint()),UVM_LOW)
  finish_item(req);
...
task thr_em_1::body();
  ...
  // 5 IER [Received Data available interrupt] 0 bit = 1 enabled
  start_item(req);
  assert(req.randomize() with {PADDR == 1; PWRITE == 1; PWDATA == 8'b0000_0010;});
    //`uvm_info("full_duplex_seqs_1",$sformatf("Printing from uart_sequence \n %s",req.sprint()),UVM_LOW)
  finish_item(req);
...
task thr_em_2::body();
  ...
  // 4 LCR[7] = 0 so, the selecting of general register and [1,0] = 11 --> 8 data bits
  start_item(req);
  assert(req.randomize() with {PADDR == 3; PWRITE == 1; PWDATA == 8'b0000_0010;});
    //`uvm_info("full_duplex_seqs_2",$sformatf("Printing from uart_sequence \n %s",req.sprint()),UVM_LOW)
  finish_item(req);

  // 5 IER [Received Data available interrupt] 0 bit = 1 enabled
  start_item(req);
  assert(req.randomize() with {PADDR == 1; PWRITE == 1; PWDATA == 8'b0000_0011;});
    //`uvm_info("full_duplex_seqs_2",$sformatf("Printing from uart_sequence \n %s",req.sprint()),UVM_LOW)
  finish_item(req);

  // 6 FCR [Received Data available interrupt] [10] bit = 11 [Tx,Rx]
  start_item(req);
  assert(req.randomize() with {PADDR == 2; PWRITE == 1; PWDATA == 8'b1000_0110;});
    //`uvm_info("full_duplex_seqs_2",$sformatf("Printing from uart_sequence \n %s",req.sprint()),UVM_LOW)
  finish_item(req);
...
task orr_seq_2::body();
  ...
  begin
    start_item(req);
  assert(req.randomize() with {PADDR == 0; PWRITE == 1; });
    //`uvm_info("full_duplex_seqs_2",$sformatf("Printing from uart_sequence \n %s",req.sprint()),UVM_LOW)
  finish_item(req);
  end
...
task ti_seq_2::body();
  ...
  // 6 FCR [Received Data available interrupt] [10] bit = 11 [Tx,Rx]
  start_item(req);
  assert(req.randomize() with {PADDR == 2; PWRITE == 1; PWDATA == 8'b1100_0110;});
    //`uvm_info("full_duplex_seqs_2",$sformatf("Printing from uart_sequence \n %s",req.sprint()),UVM_LOW)
  finish_item(req);
...
endtask : body
    ...
    class errors extends base_seq;
  `uvm_object_utils(errors)
  extern function new(string name = "errors");
  extern task body();
endclass : errors

    function errors::new(string name = "errors");
  super.new(name);
endfunction : new
...
task errors::body();
  ...
  start_item(req);
  assert(req.randomize() with {PADDR == 5;
                                PWRITE == 1;
                                PWDATA == 8'hFF;});
  finish_item(req);
  
  start_item(req);
  assert(req.randomize() with {PADDR == 2;
                                PWRITE == 1;
                                PWDATA == 8'hFF;});
  finish_item(req);
endtask:body
